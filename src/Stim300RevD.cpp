...
:Stim300Base(MAX_PACKET_SIZE)
{
    ...
    this->content = RATE_ACC_INCLI_TEMP_REVD;
...
int Stim300RevD::extractPacket(const uint8_t* buffer, size_t buffer_size) const
{
...
//  	    printf("%X \n", buffer[i]);
    ...
    if (this->modes == NORMAL)
    {
        ...
        //{
            ...
            this->inertial_values.temp[3] = convertTemp2Celsius(reinterpret_cast<uint8_t*>(&(*this->currentP).atemp_x));
            this->inertial_values.temp[4] = convertTemp2Celsius(reinterpret_cast<uint8_t*>(&(*this->currentP).atemp_y));
            this->inertial_values.temp[5] = convertTemp2Celsius(reinterpret_cast<uint8_t*>(&(*this->currentP).atemp_z));

            /** Convert inclinometers temperature raw to calibrated values **/
            this->inertial_values.temp[6] = convertTemp2Celsius(reinterpret_cast<uint8_t*>(&(*this->currentP).itemp_x));
            this->inertial_values.temp[7] = convertTemp2Celsius(reinterpret_cast<uint8_t*>(&(*this->currentP).itemp_y));
            this->inertial_values.temp[8] = convertTemp2Celsius(reinterpret_cast<uint8_t*>(&(*this->currentP).itemp_z));
...
void Stim300RevD::printInfo()
{
    ...
    std::cout<<"Acc Temperature[Celsius]: "<<this->inertial_values.temp[3]<<" "<<this->inertial_values.temp[4]<<" "<<this->inertial_values.temp[5]<<"\n";
    std::cout<<"Incl Temperature[Celsius]: "<<this->inertial_values.temp[6]<<" "<<this->inertial_values.temp[7]<<" "<<this->inertial_values.temp[8]<<"\n";
...
double Stim300RevD::convertAcc2Acceleration(const uint8_t* buffer)
{
...
bool Stim300RevD::verifyChecksum(boost::uint32_t &expintCRC, boost::uint32_t &calintCRC)
{
